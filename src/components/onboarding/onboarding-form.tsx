      ...
      <div className="relative z-10 min-h-screen lg:grid lg:grid-cols-2">
        ...
        <div className="flex flex-col min-h-screen lg:bg-background/95 lg:backdrop-blur-sm relative">
          ...
          <div className="flex-1 flex flex-col justify-center p-6 lg:p-12">
            <Card className="w-full max-w-md mx-auto bg-transparent border-none shadow-none lg:bg-card/80 lg:backdrop-blur-lg lg:border lg:border-border/50 lg:shadow-xl lg:rounded-2xl flex-1 flex flex-col">
                ...
                <div className="w-full">
                  {isCompleting ? (
                    <div className="text-center space-y-4 animate-in fade-in duration-500">
                      ...
                      <h3 className="text-xl font-semibold">Finalizing your setup...</h3>
                      <p className="text-muted-foreground">Please wait while we personalize your dashboard.</p>
                    </div>
                  ) : isComplete ? (
                    <div className="text-center space-y-6 animate-in fade-in zoom-in-95 duration-500">
                        <div className="relative w-24 h-24 mx-auto">
                            ...
                            <Sparkles className="absolute bottom-0 -left-4 w-6 h-6 text-yellow-400 animate-pulse" style={{animationDelay: '0.5s'}} />
                        </div>
                        <h2 className="text-3xl font-bold">Onboarding Complete!</h2>
                        <p className="text-lg text-muted-foreground max-w-sm mx-auto">
                            You're all set. Welcome to Fluxpense, your new home for effortless expense management.
                        </p>
                        <Button 
                            size="lg" 
                ...
                <CardFooter className="flex justify-between mt-auto">
                  ...
                  <div className="flex items-center gap-4">
                      ...
                      <Button onClick={handleNext} disabled={isLoading} className="button-glow">
                          {isLoading && step === totalSteps ? <Loader2 className="animate-spin mr-2" /> : null}
            ...
            <div className="bg-white/90 dark:bg-card/90 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-white/20">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white font-bold">
                  {isComplete ? <CheckCircle/> : step}
                </div>
                <div>
                  <div className="font-semibold text-foreground">
                    {isComplete ? "Setup Complete" : `Step ${step} of ${totalSteps}`}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {isComplete && "You're ready to go!"}
                    {step === 1 && !isComplete && "Getting Started"}
                    {step === 2 && !isComplete && "Account Type"}
                    {step === 3 && !isComplete && "Email Integration"}
                    {step === 4 && !isComplete && "Budget Setup"}
                    {step === 5 && !isComplete && "Team Collaboration"}
                  </div>
                </div>
              </div>
            </div>
